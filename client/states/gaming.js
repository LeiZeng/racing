  ...
  initializePlayers: function () {
    for (var i in this.players) {
      ...
      if (data.id == this.playerId) {
        player = new Player(RACER_ICON_WIDTH + 5, 150 * i, data.id, data.color);
      } else {
        this.remotePlayers[data.id] = new RemotePlayer(RACER_ICON_WIDTH + 5, 150 * i, data.id, data.color);
      }
    }
  },
